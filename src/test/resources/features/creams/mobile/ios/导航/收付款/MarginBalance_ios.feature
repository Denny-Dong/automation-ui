...
Feature: Title of your feature
  ...
  @CreamsAPPIOS  @wangshengwei @MAR-7096 @MAR-7146
  Scenario Outline: 搜索保证金账单
    ...
    And ios账单的信息显示正确 "<对方名称>" "<房号>" "<需收金额>" "<逾期状态>" "<应收时间>" "<楼宇>" "<账单金额>" "<实收金额>" "<费用类型>"
    And ios账单逾期状态的内容及天数显示正确 "<对方名称>"

    Examples: 
      |        邮箱        |   密码   |       楼宇       |   对方名称    | 房号 |     需收金额     |逾期状态| 应收时间 | 账单金额 |实收金额| 费用类型|
      |xypeng@91souban.com| Pexy6666 |自动化测试（不要动）|自动化测试保证金 |1-100|需收￥8500.00 | 未结清 |2018/02/06|10000.00| 1000.00 |租金保证金|
      
  @CreamsAPPIOS  @wangshengwei @MAR-7166
  Scenario Outline: 进入详情页，检查详情内容
    ...
    Then 进入账单详情页
    And ios账单信息字段显示正确 "<账单金额>" "<实收金额>" "<需收金额>" "<保证金余额>" "<费用类型>" "<账单状态>" "<账单编号>" "<计费周期>" "<应收时间>" "<创建时间>" "<跟进人>" "<备注>"
    And ios付款方信息字段显示正确 "<对方名称>" "<付款方联系人>" "<合同编号>"
    ...
    Examples: 
      ...
      |xypeng@91souban.com| Pexy6666 |自动化测试（不要动）|自动化测试保证金 |10000.00 | 1000.00 |8500.00 |1000.00 |租金保证金 |未结清 |  账单编号 |2018/02/08-2018/03/08 |2018/02/06|2018/04/16 |小破孩  |自动化测试保证金| silence |DT666666-2018-0001| 1层   100室|  100.00㎡ |贷（收入）|1000.00|1000.00|2018/02/08|自动化|500.00| 特批调整|2018/02/08|自动化500|

      
  @CreamsAPPIOS2  @wangshengwei  @MAR-7584 @MAR-7585 @MAR-7586
  Scenario Outline: 进入详情页，添加备注
    Given 输入邮箱 "<邮箱>"和密码 "<密码>"成功登录CreamsAPP
    And 点击左上角入口进入楼宇侧滑栏
    And 选择楼宇 "<楼宇>"
    And 点击右侧空白处进入楼宇 "<楼宇>" 列表
    ...
    Examples: 
       |        邮箱       |   密码   |    楼宇   |   对方名称    |    备注内容     |  姓名  |新备注内容|最新备注内容|
       |xypeng@91souban.com|Pexy6666|自动化测试（不要动）|自动化测试保证金 |自动化账单备注功能|小破孩（本人）|编辑备注功能|编辑备注功能自动化账单备注功能|
       
  
  
  @CreamsAPPIOS2  @wangshengwei @MAR-7723
  Scenario Outline: 进入详情页，合同跳转
    ...
    |xypeng@91souban.com|Pexy6666|自动化测试（不要动）|自动化测试保证金 |DT666666-2018-0001|自动化合同|
    
    
  @CreamsAPPIOS2  @wangshengwei @MAR-7724
  ...
  @CreamsAPPIOS @wangshengwei @MAR-7723
